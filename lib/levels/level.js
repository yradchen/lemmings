  ...
  handleClick(e) {
    ...
    if (this.diggers > 0) {
      ...
      for (var i = 0; i < this.lemmings.length; i++) {
        ...
        if ( (xPosition >= lemming.leftSide & xPosition <= lemming.rightSide) & (yPosition >= lemming.top & yPosition <= lemming.bottom) ) {
          let floor = this.floors.filter( floor => {
            return floor.startY === lemming.bottom;
          });
          floor = floor[0];
          let timeOut = 300;
          if (floor) {
            ...
            floor.handleHole(lemming.x, timeOut);
          }
