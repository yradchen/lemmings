...
class Lemming {
  constructor(startX, startY, stage, lastStaticObject) {
  ...
  handleFloors(floors) {
    ...
    for (let i = 0; i < floors.length; i++) {
      ...
      // if (floor.holes[this.lemming.x]) {
      //   this.lemming.y += floor.holes[this.lemming.x];
      //   this.handleHoleFalling(floor);
      //   break;
      // }
      const floorHoleDepth = floor.holes[this.lemming.x];
      if (floorHoleDepth) {
        this.handleHoleFalling(floorHoleDepth, floor.startY, floor);
        // if (floorHoleDepth + floor.startY === lemmingBottom) {
        //   this.verticalDirection = false;
        //   debugger
        //   // this.dig = true;
        //   break;
